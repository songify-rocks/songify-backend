...
async fn get_song(pool: &State<Pool<MySql>>, params: QueueParams) -> Result<Json<Value>, Status> {
    ...
    if params.full.unwrap_or(false) {
        ...
        Ok(Json(json!(song.song))) // Return only the song title as JSON
    }
}
...
async fn get_canvas(
    ...
    // Build the URL to query the canvas downloader
    let url =
        format!("https://www.canvasdownloader.com/canvas?link=https://open.spotify.com/track/{}", id);
    ...
    if response.status().is_success() {
        ...
        // Spawn a blocking task for HTML parsing (which might not be Send)
        let canvas_url = tokio::task
            ::spawn_blocking(move || {
                let document = Html::parse_document(&body);

                // Correct selector for the video tag with id="canvas_vid" and source tag
                let selector = Selector::parse("video#canvas_vid source").unwrap();
                document
                    .select(&selector)
                    .next()
                    .and_then(|source_tag| source_tag.value().attr("src"))
                    .map(|src| src.to_string())
            }).await
            .map_err(|err| err.to_string())?; // Await the result of the blocking task
